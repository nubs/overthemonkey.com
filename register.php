...
{
	$result=mysql_query("SELECT COUNT(*) AS num FROM user WHERE name='".$_POST["username"]."'") or die(mysql_error());
	$row=mysql_fetch_array($result);
		
	if($row["num"]>0)
	{	
		echo "<form action=\"http://www.overthemonkey.com/register.php\" method=\"post\">\n";
		echo "<table class=\"header\" align=\"center\"><tr><td class=\"header\">Register</td></tr><tr><td bgcolor=\"".$colors["color_bg_td_1"]."\">Username taken.</td></tr>\n";
		...
		echo "<tr><td bgcolor=\"".$colors["color_bg_td_1"]."\" width=\"30%\">Password:</td><td bgcolor=\"".$colors["color_bg_td_1"]."\" width=\"70%\" align=\"right\"><input type=\"password\" name=\"password\" size=\"50\" maxlength=\"15\"></td></tr>\n";
		echo "<tr><td colspan=\"2\" align=\"right\"><input type=\"submit\" value=\"submit\" name=\"submit\"></td></tr></table></td></tr></table></form>\n";
	}
	else
	{
		$result=mysql_query("INSERT INTO user VALUES(NULL, '".addslashes($_POST['username'])."', '".md5($_POST['password'])."', 0, '', NULL, NULL, NULL, NULL, NULL, 1, NULL)") or die(mysql_error());
		
		$result=mysql_query("SELECT id, name FROM user WHERE name=\"".addslashes($_POST["username"])."\"") or die(mysql_error());
		$row=mysql_fetch_array($result);
	...
	fwrite($file,"		echo \"<tr><td bgcolor=\".\$color.\" width=\\\"55%\\\" onMouseOver=this.style.backgroundColor=\\\"\".\$colors[\"color_bg_th\"].\"\\\" onMouseOut=this.style.backgroundColor=\\\"\\\" onclick=\\\"window.location.href='http://www.overthemonkey.com/viewmessage.php?id=\".\$row[\"id\"].\"'\\\"><a class=\\\"small\\\" href=\\\"http://www.overthemonkey.com/viewmessage.php?id=\".\$row[\"id\"].\"\\\">\".parse(\$row[\"title\"]).\"</a></td><td class=\\\"small\\\" bgcolor=\".\$color.\" width=\\\"10%\\\" align=\\\"center\\\">\".(\$row[\"views\"]>0?\$row[\"views\"]:\"\").\"</td><td class=\\\"time\\\" bgcolor=\".\$color.\" width=\\\"35%\\\">\".us_date(\$row[\"date\"]).\" \".us_time(\$row[\"date\"]).\"</td></tr>\\n\";\n");
		...
		echo "<table class=\"header\" align=\"center\"><tr><td class=\"header\">Register</td></tr><td bgcolor=\"\".\$colors[\"color_bg_td_1\"].\"\">User account created<br><a href=\"http://www.overthemonkey.com/login.php\">Login</a></td></tr></table>";
	}
}
else
{
	echo "<form action=\"http://www.overthemonkey.com/register.php\" method=\"post\">\n";
	echo "<table class=\"header\" align=\"center\"><tr><td class=\"header\">Register</td></tr>\n";
	echo "<tr><td><table width=\"100%\"><tr><td bgcolor=\"".$colors["color_bg_td_1"]."\" width=\"30%\">Username:</td><td bgcolor=\"".$colors["color_bg_td_1"]."\" width=\"70%\" align=\"right\"><input type=\"text\" name=\"username\" size=\"50\" maxlength=\"15\"></td></tr>\n";
	echo "<tr><td bgcolor=\"".$colors["color_bg_td_1"]."\" width=\"30%\">Password:</td><td bgcolor=\"".$colors["color_bg_td_1"]."\" width=\"70%\" align=\"right\"><input type=\"password\" name=\"password\" size=\"50\" maxlength=\"15\"></td></tr>\n";
	echo "<tr><td colspan=\"2\" align=\"right\"><input type=\"submit\" value=\"submit\" name=\"submit\"></td></tr></table></td></tr></table></form>\n";
}
